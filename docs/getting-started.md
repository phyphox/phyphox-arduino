...
| senseBox MCU / MCU-S2 with the Bluetooth-Bee (NINA-B31) | `poll()` required. The module's UART is `Serial3` on the senseBox MCU core and `Serial1` on the MCU-S2; another port: `-DPHYPHOX_BLE_NINA_SERIAL=SerialX`. If the board never appears in the app, call `PhyphoxBLE::printErrors(&Serial)` after the first `poll()` (the module is configured then): it says when the module did not answer. `PhyphoxBLE::begin(&Serial)` before `start()` (or `-DPHYPHOX_BLE_NINA_TRACE`) prints the whole AT dialogue with the module on `Serial` — the thing to send when reporting a problem. Supported with the senseBox colleagues' testing |
